...
void adventurerAction(int currentPlayer, struct gameState *state){
  ...
  //BUG-drawntreasure++ was changed to drawntreasure--
  //whenever adventurer is played, program will hang 
    drawntreasure--;
  else{
...
void smithyAction(int currentPlayer, struct gameState *state, int handPos){
  int i;
    //BUG-changed i from 0 to 1
    //player gains one less card than expected
      //+3 Cards
      for (i = 1; i < 3; i++)
  {
    drawCard(currentPlayer, state);
...
void villageAction(struct gameState *state, int handPos){
    int currentPlayer = whoseTurn(state);
        //+1 Card
      drawCard(currentPlayer, state);
  //BUG - numActions + 2 was changed to just numActions
  //players +2 action effect is now gone   
      //+2 Actions
...
void great_hallAction(struct gameState *state, int handPos){
    int currentPlayer = whoseTurn(state);
        //+1 Card
      drawCard(currentPlayer, state);
...
void outpostAction(struct gameState *state, int handPos){
    int currentPlayer = whoseTurn(state);
    //BUG - ++ from outpostPlayed++ was removed
    //outpost card would have no action even if played
        //set outpost flag
      state->outpostPlayed;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  // moved due to refactor of advernturer
  // int drawntreasure=0;
  // int cardDrawn;
  ...
  switch( card ) 
    ...
    case great_hall:
    great_hallAction(state, handPos);
    return 0;
